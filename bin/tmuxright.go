...
func main() {
	...
	cmd2 := exec.Command("sh", "-c", `uptime | awk '{print $10 " " $11 " " $12}'`)
	//cmd2.Start()
	...
	if err == nil {
		...
		uptimes[0], _ = strconv.ParseFloat(parts[0], 64)
		uptimes[1], _ = strconv.ParseFloat(parts[1], 64)
		uptimes[2], _ = strconv.ParseFloat(parts[2], 64)
	} else {
		uptimes[0] = 42
		uptimes[1] = 42
		uptimes[2] = 42
